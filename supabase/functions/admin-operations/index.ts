...
serve(async (req) => {
  ...
  try {
    ...
    const superAdminOps = ['disableUser', 'enableUser', 'deleteUser', 'setTrialPeriod', 'activatePermanent', 'getAdminStats', 'cancelPlan', 'changeTenantPlan', 'updatePlanLimits']
    ...
    switch (body.operation) {
      ...
      case 'sendPasswordReset': {
        ...
        if (resetError) {
          ...
          return new Response(JSON.stringify({ error: 'Failed to send reset email: ' + resetError.message }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
        }
